...
class XPage extends HTMLElement {
    ...
    async loadPage(settings) {
        ...
        //loading
        let loadingComponentName = config.get("ui.components.page-loading");
        ...
        if (!response.ok) {
            ...
            //add default metas
            debugger;
            for (let key of config.getKeys("ui.meta.")) {
                let meta = { name: key.substring("ui.meta.".length), value: config.get(key) }
            ...
            //layout
            let layout = config.get("ui.layouts.page.default");
            doc.head.querySelectorAll("meta[name='x-page-layout']").forEach((sender) => { layout = config.get("ui.layouts.page." + sender.content); });
            if (this._type) layout = config.get("ui.layouts.page." + this._type);
            if (searchParams.get("x-page-layout")) layout = config.get("ui.layouts.page." + searchParams.get("x-page-layout"), layout);
    ...
    async _showError(code, message, stacktrace, container) {
        var name = config.get("ui.components.page-error");
