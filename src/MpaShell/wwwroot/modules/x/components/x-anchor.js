    ...
    methods: {
        ...
        onCommand(command, args) {
            if (command == "load") {
                ...
                //refresh
                if (this.state.href) {
                    let mpaShell = this.mpaShell;
                    if (mpaShell) {
                        this.state.realHref = mpaShell.getRealUrl(this.state.href, this.page, {breadcrumb: this.state.breadcrumb, type: this.state.target});
                    }
                } else {
                ...
                if (this.state.command) {
                    ...
                    //target mpa-page element
                    if (this.state.target == "#stack") {
                        mpaShell.showPage({ url: this.state.href, target: "#stack"});
                        event.preventDefault();
                        event.stopPropagation();
                        return false;
                    } else if (this.state.target == "#dialog") {
                        mpaShell.showPage({ url: this.state.href, target: "#dialog"});
                        event.preventDefault();
                        event.stopPropagation();
                        return false;
                    ...
                    //page
                    let src = mpaShell.getRealUrl(this.state.href, this.page, {breadcrumb: this.state.breadcrumb, relative: true});
                    if (mpaShell.navigator.pages[0] == this.page) {
                        mpaShell.showPage({url: src});
                    } else {
                        this.page.src = src;
                    }
                    event.preventDefault();
